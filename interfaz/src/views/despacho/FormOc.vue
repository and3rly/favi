<template>
  <div v-if="actual == 1">
    <div class="alert alert-info p-2 mb-2">
      <i class="fas fa-lightbulb me-1"></i> Seleccione un pedido para poder cargar su detalle.
    </div>
      ...
      <table class="table table-hover table-bordered table-sm m-0">
        ...
        <tbody>
          ...
          >
            ...
            <td class="text-center">{{ formatoFecha(i.fecha_pedido, 2) }}</td>
            <td>{{ i.nombre_tipo }}</td>
  ...
  export default {
    ...
    methods: {
      getListaOc() {
        ...
        this.$http
        .get(`${this.$baseUrl}/pedido/pedido/buscar`, {params: this.bform})
        .then(res => {
          this.inicio = false
          if (res.data.lista) {
      ...
      getDetalle(obj) {
        ...
        this.$http
        .get(`${this.$baseUrl}/pedido/detalle/pedidodetalle/buscar`, {params: { pedido_enc_id: obj.id }})
        .then(res => {
          this.inicio = false
          if (res.data.lista) {
      ...
      guardar() {
        ...
        this.$http
        .post(`${this.$baseUrl}/despacho/detalle/guardar_detalle`, this.form)
        .then(result => {
